<template>
  <div class="flex flex-col h-screen w-screen bg-gray-800">
    <div class="flex flex-row-reverse p-4 gap-4 text-white">
      <button @click="toggleEditMode" class="px-4 py-2 border border-slate-600 hover:bg-slate-600 rounded-md text-white">
        {{ editMode ? 'Disable' : 'Enable' }} Edit Mode
      </button>
      <button @click="addWidget" class="px-4 py-2 border border-slate-600 hover:bg-slate-600 rounded-md text-white">
        Add Widget
      </button>
    </div>
    <div class="flex-1 p-4">
      <div class="grid-stack" ref="gridContainer">
        ...
        >
          <div
            class="grid-stack-item-content cursor-pointer flex flex-col gap-2 justify-center items-center bg-slate-600 hover:bg-cyan-800 text-white"
          >
            <label>{{ widget.title }}</label>
            <button v-if="editMode" @click="removeWidget(widget.id)" class="ml-1 text-center py-1 px-2 rounded-md bg-red-500">
              Remove
            </button>
          </div>
        </div>
    ...
    (newWidgets) => {
      newWidgets.forEach((widget) => {
        ...
        if (!element) {
          ...
          widgetContent.className =
            'grid-stack-item-content flex flex-col gap-2 justify-center items-center bg-slate-600 hover:bg-cyan-800 text-white';
